...
public:
  ...
  void imgMat(const cv::Mat& img, double ts);
...
void VoNode::imgMat(const cv::Mat& img, double ts)
{
  ...
  processUserActions();
  vo_->addImage(img, ts);
  visualizer_.publishMinimal(img, vo_->lastFrame(), *vo_, ts);
...
int main(int argc, char **argv)
{
  ...
  /*std::cout << "creating cap..." << std::endl;
  cv::VideoCapture cap(0);
  double ts = 0;
  std::cout << "created cap." << std::endl;*/

  // subscribe to remote input
  vo_node.sub_remote_key_ = nh.subscribe("svo/remote_key", 5, &svo::VoNode::remoteKeyCb, &vo_node);
  std::cout << "created remote_key." << std::endl;

  // start processing callbacks
  while(ros::ok() && !vo_node.quit_)
  {
    ros::spinOnce();
    //std::cout << "spined." << std::endl;
